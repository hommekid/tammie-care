...
- **การรักษา** (key `treat`) = ปฏิทิน (จุด/กรอบสีทอง) · จิ้มวันมีการรักษา → กล่องรายละเอียดขึ้นข้างๆ (หมอ—แผนก+บันทึก แต่ละ entry) · **แต่ละ entry แสดงรูปแนบ (lightbox) + flowchart Mermaid ถ้ามี** (ผ่าน `txEntryHTML`/`txFlow`/`renderMermaid`)
  - **ทุกปฏิทิน (nosebleed/treat/daily) โชว์เฉพาะรายการของวันที่จิ้มเท่านั้น — ไม่จิ้ม = ไม่โชว์ (เอารายการประวัติทั้งหมดใต้ปฏิทินออกแล้ว: `txList`/`dyList`/`nb-list`)** · กล่องรายละเอียด**ตีกรอบ+เงา** · จิ้มวันเดิมซ้ำ = **toggle ซ่อน** · ใช้ id แยกกัน (txCal* / dyCal*) ไม่ชนกัน · (ลบ dead code แล้ว: helper `txListItem`/`txToggleAcc`/`dailyListItem` + CSS `.nb-list`/`.tl-acc*`)
  - **ทุกปฏิทิน (nosebleed/treat/daily) เปิดที่เดือนปัจจุบันเสมอ + ไฮไลต์วันนี้ (Today)**: helper กลาง `TODAY_ISO`/`TODAY_MK` + `calMonthsWithToday()` (แทรกเดือนปัจจุบันเข้าลิสต์เดือนถึงไม่มีข้อมูล) + `todayIdx()` (ตั้ง idx เริ่มที่เดือนนี้) · ช่องวันนี้ได้คลาส `.today` (กรอบฟ้า outline)
...
- **ทุกหมวดมี list + เพิ่ม/แก้/ลบ inline ในแท็บตัวเอง** (ไม่มีเมนู "แก้ไข/ลบ รายการเดิม" รวมแล้ว) · กดแก้ = ดึงขึ้นฟอร์ม ปุ่มเปลี่ยนเป็น "บันทึกการแก้ไข", กดลบ = ยืนยันก่อน
  ...
  - ปฏิทิน admin ใช้ helper กลาง `calRender/calNav/calPick/calSel` + `calState` (ล้างตอนสลับสัตว์) · กันไม่ให้ refresh ทับช่องวันที่ตอนกำลังแก้ (เช็ก `symEdit[src]`/`trEdit`) · **เปิดที่เดือนปัจจุบันเสมอ (base = `new Date()`) + ไฮไลต์วันนี้ผ่านคลาส `.today` (`CAL_TODAY_ISO`)**
...
- CBC 9 ครั้ง (10/25 ม.ค., 3/16/24 ก.พ., 30 พ.ค., 14/20/27 มิ.ย.) · เคมี 5 ครั้ง (10 ม.ค., 24 ก.พ., 30 พ.ค., 14/20 มิ.ย.)
- Cardiac 2 (14 มิ.ย. Troponin 0.31 ผิดปกติ · 20 มิ.ย. Troponin 0.07 ปกติ) · Coagulation 1 (20 ม.ค.)
- ล่าสุด 27 มิ.ย.: WBC 4,030 (Leuco Plus 🟡 ดีขึ้นจาก 🔴 3,310), ซีดน้อยลง (HGB 12.2 จาก 9.9, HCT 37.1) · MONO/LYM ยังต่ำเล็กน้อย
...
- ยาประจำ 5 รายการ · CBC 4 ครั้ง + เคมี 5 ครั้ง (24 ก.พ. / 2 พ.ค. / 30 พ.ค. / 20 มิ.ย. / 27 มิ.ย. 2026) — labConfig/labPanels ใช้ค่าอ้างอิง VetCal เหมือนเฟ่
- 27 มิ.ย.: ใบ chem วัดเฉพาะ Total Cholesterol 178 / Triglyceride 52 (ปกติทั้งคู่) · comment Plasma protein 5.0
- ล่าสุด (ค่าครบ) 20 มิ.ย.: โปรตีนยังต่ำ (Albumin 2.0, Total Protein 4.5), WBC/LYM ต่ำ
