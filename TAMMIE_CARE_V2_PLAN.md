...
## 🧹 ปรับ UX การจัดการให้สม่ำเสมอ (✅ แก้ครบ 7 ข้อ 20 ก.ค. 2026)

| # | ปัญหา | แก้แล้วเป็น |
|---|---|---|
| 6 | ลบหมวดผลเลือดเตือนครั้งเดียว | ✅ เตือน 2 ชั้น + พิมพ์ชื่อหมวดยืนยัน (ถ้ามีผลตรวจ) |
| 1 | "ซ่อน" หลายที่ ไม่เหมือนกัน | ✅ ใช้คำ "ซ่อน/แสดง" ตรงกัน · "⚙️ จัดการหมวด" → "⚙️ ซ่อน/แสดงหมวด" |
| 2 | การแก้ไม่มี pattern เดียว | ✅ ยึดกติกา ✏️=แก้เนื้อหา · ⚙️=ตั้งค่า/โครง · done-label "✓ เสร็จ" |
| 3 | ⚙️ เฟืองทำหลายอย่าง | ✅ เฟืองมีข้อความกำกับชัดทุกที่ · รวมชื่อ "⚙️ ค่าตรวจ + เกณฑ์ปกติ" ให้ตรงกัน |
| 5 | ⚙️ ค่า+เกณฑ์ผลเลือดหายาก | ✅ ย้าย ✏️ แก้ผลล่าสุด + ⚙️ ค่าตรวจ ขึ้นหัวหมวด (เห็นทันทีที่กาง) |
| 4 | แก้โปรไฟล์อยู่ลึก | ✅ หน้าจัดการครอบครัว รายการสัตว์กดชื่อ/✏️ แก้ข้อมูล เข้า dashboard ได้ |
| 7 | เพิ่มเทมเพลตซ้ำได้ | ✅ เช็กชื่อหมวดซ้ำก่อนเพิ่ม เตือนก่อน |

> ข้อ 8 เดิม (ตารางค่า+เกณฑ์บนมือถือ) — ตรวจแล้วโอเค ตัดออก

**ปรับต่อหลังเทส 7 ข้อ (20 ก.ค. 2026):**
- หน้าจัดการสัตว์: ปุ่ม ✏️ แก้โปรไฟล์ / ซ่อน / ลบถาวร ทำเป็น**ปุ่มจริง** (มีสี/ขอบ) + tooltip กดค้างบนมือถือ (อ่านจาก `title`, อยู่ใน lib.js ใช้ได้ทุกหน้า) + จัดปุ่มบนมือถือเป็นชื่อบรรทัดบน/3 ปุ่ม grid เท่ากัน
- ✏️ แก้โปรไฟล์ ในหน้าจัดการ → เปิดฟอร์มแก้โปรไฟล์ให้เลย (`?edit=profile`)
- **ผลเลือด: ย้าย "➕ เพิ่มผลตรวจ" เข้าหัวหมวดที่กาง** (ฟอร์มโผล่ในหมวด ไม่เด้งขึ้นบน ไม่ต้องเลือกหมวดใหม่) · เอาปุ่มเพิ่มผลด้านบนออก เหลือ "➕ เพิ่มหมวด" + "⚙️ ซ่อน/แสดงหมวด"

**ปรับเพิ่ม 12 ส.ค. 2026:**
- **กล่องแจ้งเตือนของเราเอง หัวเป็น "🐾 Tammie says"** — ทับ `window.alert` ที่ `lib.js` ครอบคลุมทุกจุดที่แจ้ง error/เตือน (28 จุด) ไม่ต้องแก้ทีละที่ · เบราว์เซอร์บังคับหัวข้อ alert เป็นชื่อโดเมน เปลี่ยนจาก JS ไม่ได้ · ⚠️ กล่องใหม่ไม่บล็อก ดู `NOTES_gotchas.md` กับดักข้อ 8 · `confirm`/`prompt` ยังเป็นของเบราว์เซอร์
...
- **หน้าสรุป: สัตว์ใหม่เริ่มด้วยแค่ 🚨 danger + 📋 watch** — แยก section เป็น core (มีเสมอ) + เทมเพลต leuco/tumor (เลือกเพิ่ม/ลบได้เหมือนเทมเพลตผลเลือด ผ่าน ⚙️ ปรับหน้าสรุป) · เฟ่/เฟ่อเก่ามีข้อมูลอยู่แล้วจึงโชว์อัตโนมัติ ไม่ต้อง migrate ดู `NOTES_gotchas.md` กับดักข้อ 10
...
### 🔎 ระบบขอเข้าครอบครัว + ตั้งชื่อครอบครัว (จด 20 ก.ค. 2026 — ยังไม่ทำ)

> 📄 **แผนฉบับเต็มแตกเป็นไฟล์แยกแล้ว: [`JOIN_FLOW_PLAN.md`](JOIN_FLOW_PLAN.md)** (12 ส.ค. 2026)
> — schema `join_requests` + รหัสครอบครัว · RLS · RPC 10 ตัว · UI 5 ไฟล์ · ลำดับลงมือ 8 step · test checklist 10 ข้อ
> ข้างล่างนี้คือบันทึกตั้งต้นเดิม เก็บไว้เป็นที่มา

รอทดสอบ UX 7 ข้อให้เสร็จก่อน แล้วค่อยเริ่ม

- **ตั้งชื่อครอบครัว** — admin แก้ชื่อครอบครัวได้ (ตอนนี้ตั้งครั้งเดียวตอน seed) · มี RLS `families_update` (admin) อยู่แล้ว แค่ทำ UI
- **ค้นหา + ขอเข้าครอบครัว** — คนที่สมัครใหม่ (parent) หรือสัตวแพทย์ ค้นหาชื่อครอบครัว → กดขอเข้า → **แอดมินของครอบครัวนั้นเป็นคน approve** (parent เข้าเป็นสมาชิก · vet เข้าเป็นสิทธิ์ดู)
  - ต้องมีตารางใหม่ `join_requests` (family_id, user_id, role_requested, status) + RLS + RPC (ขอ/อนุมัติ/ปฏิเสธ)
  - **มี 2 ทางเข้า แต่ปลายทางต้องยืนยันเสมอ:**
    - ผู้ใช้ค้นหา+ขอเข้าเอง → **admin กด approve**
    - admin เพิ่มด้วยอีเมลเอง (แบบปัจจุบัน) → **ผู้ถูกเพิ่มต้องกดยอมรับคำเชิญ** ก่อนถึงจะเข้าจริง
    - สรุป: ไม่ว่าใครเริ่ม อีกฝ่ายต้องกดยืนยัน — กันทั้งคนแอบขอเข้า และกันการถูกลากเข้าครอบครัวโดยไม่รู้ตัว · สถานะใน `join_requests`: pending → approved/rejected · ทิศทางคำขอ (`requested_by`: user หรือ admin) บอกว่าใครต้องเป็นคนกดยืนยัน
  - หมายเหตุความเป็นส่วนตัว: การค้นหาครอบครัวได้ = คนนอกเห็นชื่อครอบครัว → ควรให้เห็นแค่ชื่อ ไม่เห็นสัตว์/สมาชิก จนกว่าจะ approve
